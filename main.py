...
def generate_runscript_from_code (workdir=".", environment=None, pre_instruction=None, instruction="run", outputs_folder_path="."):
    ...
    runscript_file.write("watchmedo shell-command --command='echo \"${watch_src_path} ${watch_dest_path}\" >> ~/watchdog_log.txt' --patterns=\"*\" --ignore-patterns='watchdog_log.txt;run_stderr.txt;run_stdout.txt' --ignore-directories --recursive " + str(outputs_folder_path) + " & WATCHDOG_PID=$!;\n\n")
...
if __name__ == "__main__":
    ...
    if (not runscript_file):
        runscript_file = generate_runscript_from_code (workdir, environment, pre_instruction, instruction, outputs_folder_path=outputdir)
